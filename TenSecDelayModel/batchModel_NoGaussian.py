    ...
    def calcBPM(self):
        ...
        for i, frame in enumerate(currframes):
            ...
            for k in range(levels - 1):
                u = cv2.pyrUp(currframes[k + 1])
                (height, width, depth) = u.shape
                currframes[k] = cv2.resize(currframes[k], (height, width))
                diff = cv2.subtract(currframes[k], u)
                laplacian_pyramid.append(diff)

            laplacian_pyramid.append(currframes[-1])
